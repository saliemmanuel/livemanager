...
def check_django_service():
    ...
    try:
        ...
        result = subprocess.run(['systemctl', 'is-active', 'livemanager'], 
                              capture_output=True, text=True, timeout=10)
        ...
        result = subprocess.run(['journalctl', '-u', 'livemanager', '--no-pager', '-n', '10'], 
                              capture_output=True, text=True, timeout=10)
...
def main():
    ...
    if tests_passed == total_tests:
        ...
        print("   1. Vérifier les permissions: sudo chown -R www-data:www-data /var/www/livemanager")
