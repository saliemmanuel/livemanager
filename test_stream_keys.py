...
def test_stream_key_creation():
    ...
    try:
        ...
        print(f"✅ Clé de streaming sauvegardée avec succès!")
    ...
    try:
        ...
        print(f"✅ Clé trouvée dans la base de données")
        print(f"   Nom: {saved_key.name}")
        print(f"   Clé: {saved_key.key[:50]}...")
        
    except StreamKey.DoesNotExist:
        print(f"❌ Clé non trouvée dans la base de données")
        return False
    except Exception as e:
    ...
    try:
        ...
        for key in user_keys:
            print(f"   - {key.name} ({key.platform}) - {'Active' if key.is_active else 'Inactive'}")
...
def test_stream_key_validation():
    ...
    print(f"📝 Test avec données invalides:")
    print(f"   Validité: {form.is_valid()}")
    if not form.is_valid():
